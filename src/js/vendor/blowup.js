...
(function ($) {
  $.fn.blowup = function (attributes) {
    ...
    $element.mousemove(function (e) {
      ...
      // Constants
      const $IMAGE_URL    = $('.active').attr('src');
      const NATIVE_IMG    = new Image();
      NATIVE_IMG.src    = $('.active').attr('src');
      ...
      var zoomX =-6 -Math.floor(relX / $(this).width() * (NATIVE_IMG.width * $options.scale) - $options.width / 2);
      var zoomY =-6 -Math.floor(relY / $(this).height() * (NATIVE_IMG.height * $options.scale) - $options.height / 2);
